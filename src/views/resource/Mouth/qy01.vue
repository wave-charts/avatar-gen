<template>
  <svg
    :width="width"
    :height="height"
    viewBox="0 0 380 380"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M230.37 232.604C230.537 230.398 228.426 229.081 226.628 229.568C224.222 230.22 219.129 231.481 215.571 231.608C211.639 231.748 205.915 230.634 203.322 230.069C201.498 229.671 199.475 231.093 199.736 233.271C199.917 234.777 200.319 236.25 200.932 237.643C201.747 239.493 202.918 241.164 204.378 242.562C205.839 243.959 207.56 245.056 209.445 245.788C211.329 246.52 213.339 246.874 215.36 246.83C217.381 246.785 219.373 246.343 221.223 245.528C223.073 244.714 224.745 243.543 226.142 242.082C227.54 240.622 228.636 238.9 229.368 237.016C229.919 235.6 230.255 234.113 230.37 232.604Z"
      fill="#171921"
      stroke="#171921"
      stroke-width="4"
    />
    <mask
      id="mask0"
      mask-type="alpha"
      maskUnits="userSpaceOnUse"
      x="198"
      y="231"
      width="33"
      height="16"
    >
      <path
        d="M230.814 231.39C230.858 233.357 230.487 235.313 229.722 237.148C228.958 238.983 227.816 240.66 226.36 242.083C224.904 243.507 223.164 244.649 221.239 245.444C219.314 246.24 217.241 246.673 215.139 246.719C213.037 246.766 210.947 246.424 208.989 245.714C207.03 245.004 205.242 243.94 203.725 242.582C202.208 241.224 200.993 239.599 200.148 237.8C199.304 236.001 198.847 234.062 198.803 232.095L230.814 231.39Z"
        fill="#171921"
      />
    </mask>
    <g mask="url(#mask0)">
      <ellipse
        cx="215.45"
        cy="246.436"
        rx="10.7563"
        ry="10.065"
        transform="rotate(-1.26265 215.45 246.436)"
        fill="#FC909F"
      />
    </g>
    <path
      d="M218.237 231.222C218.279 233.154 216.3 234.765 213.815 234.82C211.33 234.875 209.281 233.353 209.239 231.42C209.196 229.488 211.231 230.376 213.716 230.321C216.2 230.266 218.194 229.289 218.237 231.222Z"
      fill="white"
    />
  </svg>
</template>

<script lang="ts">

import Component from "vue-class-component";
import {  Mixins } from "vue-property-decorator";
import BaseMixin from "@/views/base.mixin";

@Component({
  components: {},
})
export default class Mouth8 extends Mixins(BaseMixin) {}
</script>
